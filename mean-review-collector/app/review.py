...
def dashboard():
    db = get_db()
    reviews = db.execute(
        'SELECT r.id, u.id, created, author_id, content, username'
        ' FROM review r JOIN user u ON r.author_id = u.id'
        ' WHERE author_id = ?'
        ' ORDER BY created DESC',
        (g.user['id'],)
    ...
    if reviews==[]:
        reviews = db.execute(
                'SELECT u.id, username, "" as content'
                        ' FROM user u '
                                ' WHERE u.id = ?'
                                        ' ORDER BY username DESC',(g.user['id'],)
                                            ).fetchall()
    for i in reviews:
        print(dict(i))
        #reviews=dict(i)
...
def get_review(id):
    ...
    print("printintg author id and user id")
    print(review['author_id'],g.user['id'])
